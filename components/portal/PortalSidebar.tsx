...
export function PortalSidebar({ user }: PortalSidebarProps) {
  ...
  return (
    <>
      {/* ── Desktop sidebar ── */}
      <aside className="hidden md:flex flex-col w-64 min-h-screen bg-white border-r border-gray-200 shrink-0">
        {/* Logo + empresa */}
      ...
      {/* ── Mobile top header ── */}
      <header className="md:hidden fixed top-0 inset-x-0 z-40 h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4">
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest leading-none">Portal</p>
          <p className="text-sm font-bold text-gray-900 truncate max-w-[180px]">{user.companyName}</p>
        </div>
        <button
          onClick={() => signOut({ callbackUrl: '/login' })}
